...
class PascalVocWriter:
    ...
    def appendObjects(self, top):
        for each_object in self.boxlist:
            ...
            if int(float(each_object['y3'])) == int(float(self.imgSize[0])) or (int(float(each_object['y1']))== 1):
                truncated.text = "1" # max == height or min
            elif (int(float(each_object['x1']))==int(float(self.imgSize[1]))) or (int(float(each_object['x2']))== 1):
